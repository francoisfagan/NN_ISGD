...
It is to be also made clear that no knowledge of it's original project
is included or used to device this script.
This script is complete freeware developed by above signed users, and may further be
...
#     a, b, c, d = 100 * np.random.randn(4)
#     cubic_soln = np.sort(np.real(solve(a, b, c, d)))
#     np_soln = np.sort(np.real(np.roots([a, b, c, d])))
#
#     same = all(abs(cubic_soln - np_soln) < 1e-8)
#     if not same:
#         print('Solutions differ!')
#         print(a, b, c, d)
#         print(cubic_soln)
#         print(np_soln)
#         print(abs(cubic_soln - np_soln))
#         print('')
#
# print('End of test')
...
A = np.random.randn(4, 50) # np.array([0, 0, 1, 0])  #
# print(A)
a, b, c, d = A
...
def real_root_closest_to_zero(roots):
...
roots = np.apply_along_axis(np.roots, 0, A)
root_closest_to_zero = np.apply_along_axis(real_root_closest_to_zero, 0, roots)

print(roots)
print(root_closest_to_zero)
